...
const MegaCollection = () => {
  ...
  // Fetch mega collection products
  useEffect(() => {
    const fetchProducts = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`${API_BASE_URL}/api/products/category/megaCollection`);
        if (!response.ok) {
  ...
  return (
    <div className="bg-gray-50">
      <SmallHeader pageTitle="Mega Collection" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {products.length > 0 ? (
              ...
              <div className="col-span-full text-center py-10">
                <p className="text-gray-600 text-lg">No mega collection products found.</p>
              </div>
            )}
          </div>
        )}
